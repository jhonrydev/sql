ALTER PROCEDURE [saludmp].[SP_ADM_inconsisPrest]
...
AS  
	...
	IF (@app = '') BEGIN 
		SELECT @coderror = 4;
		SELECT @msgerror = 'Debe ingresar la APP que presenta la inconsistencia.';
	...
	COMMIT TRANSACTION
	SELECT @coderror = 0;
	SELECT @msgerror = 'Se Registro Exitosamente';				

    select @cod_inconsis_prestapp = scope_identity()
