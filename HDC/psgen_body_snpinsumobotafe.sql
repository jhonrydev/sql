create or replace package body psgen_snpinsumobotafe is
   ...
   begin
      ...
      if ( prm_usuario is null or prm_usuario = '' ) then
         ...
         open cu_role_user_profile;
            loop
               ...
               if(v_permiso_profile.clave = 'ROLE_COMERC_AFE' or v_permiso_profile.clave='ROLE_OPERAC_AFE' or v_permiso_profile.clave='ROLE_ADMIN_AFE') then
                  v_roles_usuario := v_roles_usuario || v_permiso_profile.clave || ',';
               end if;
            end loop;
         close cu_role_user_profile;

         v_roles_usuario:= SUBSTR(v_roles_usuario,1,length(v_roles_usuario)-1);

         if(v_roles_usuario is null or v_roles_usuario ='') then
      ...
      if(v_roles_usuario is not null and v_permiso_cambio = 1 ) then
         select valor
           into v_permiso_rol
           from app_config
