   ...
   begin
      ...
      if ( prm_motivo_actual is null or prm_motivo_actual = '' ) then
         ...
         if ( v_info_devolucion_actual.codigo is null ) then
            msgerror := 'El motivo de devolución prm_motivo_actual ('||prm_motivo_actual||') no es válido';
            coderror := 5;
            return;
         else
      ...
      if ( prm_motivo_nuevo is null or prm_motivo_nuevo = '' ) then
         ...
         if ( v_info_devolucion_nuevo.codigo is null ) then
            msgerror := 'El motivo de devolución prm_motivo_nuevo (' ||prm_motivo_nuevo|| ') no es valido!';
            coderror := 5;
            return;
         else
